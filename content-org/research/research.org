...
* TODO Adaptive Nuclear Statistical Equilibrium for Type Ia SN Simulation
:PROPERTIES:
:EXPORT_FILE_NAME: index
...
1. If there are exactly two isotopes involved in the /k-th/ reaction that are not in LIG, and if they're not in the same group then merge the isotope in the smaller group into the isotop in the larger group.
2. If there is only 1 group involved in the /k-th/ reaction that is not
