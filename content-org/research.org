...
Temperature of the cell must exceed a certain value, by default this is 4 \times 10^{9} K,
which NSE is not expected to occur at all.
...
when transitioning from a non-NSE state to NSEstate. Therefore, an abrupt change in the energy is expected.
To avoid this
